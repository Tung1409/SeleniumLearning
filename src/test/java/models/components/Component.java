...
public class Component {
    ...
    public <T extends Component> List<T> findComponents(Class<T> componentClass, WebDriver driver){
        // Get component Selectỏ
        By componentSelector;
        try {
            componentSelector = getCompSelector(componentClass);
        }catch (Exception e){
            throw new IllegalArgumentException("[ERR] component must have a css selector!");
        }
        ...
        // bien nhung cai element minh tim duoc thanh cai List cua nhung cai WebElement hoac component
